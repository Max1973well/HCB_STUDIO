...
def _gemini_generate(prompt: str, provider_cfg: dict) -> dict:
...
def generate_with_active_provider(config_path: Path, prompt: str) -> dict:
    config = load_engine_config(config_path)
    ...
    if active == "gemini":
        return _gemini_generate(prompt, provider_cfg)
